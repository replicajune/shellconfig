...
if [ "${TERM_PROGRAM}" = "vscode" ]; then
  ...
  PROMPT='$CC_DARK_GREY%*$CC_RESET_COLOR [%(!.$CC_ORANGE.$CC_GREEN)%n$CC_RESET_COLOR@$CC_CYAN%m $CC_RESET_COLOR$CC_DARK_GREY%c$CC_RESET_COLOR$(prompt_git)] %(?.$CC_DARK_GREY.$CC_RED)%?$CC_RESET_COLOR [$(prompt_load)] '$'\n''> '
fi
